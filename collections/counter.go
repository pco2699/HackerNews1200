...
func (h *counterHeap) update(item *counterItem, value string, count int) {
	item.value = value
	item.count = count
...
func (c *Counter) Get(item string) int {
...
func (c *Counter) AddItems(items ...string) {
	for _, item := range items {
		c.addItem(item)
	}
}

func (c *Counter) addItem(item string) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
